// 错误展示层：把后端英文 message / 网络异常映射为 i18n key，未命中回退后端原文。
// 页面侧存储原始 error 对象（unknown），渲染期经 displayError 解析，保证切换语言即时刷新。
import type { TFunction } from "i18next"
import { ApiError } from "@/api/client"

// 已知 HTTP 状态 → 本地化文案 key（后端 message 为英文，按状态归一化展示）
...
/** ApiError 按 status 映射；未映射状态回退后端 message（空则 fallbackKey）。非 ApiError（断网等）→ errors.network */
export function translateError(t: TFunction, err: unknown, fallbackKey: string): string {
  if (err instanceof ApiError) {
    const key = STATUS_KEY[err.status]
    if (key) return t(key)
